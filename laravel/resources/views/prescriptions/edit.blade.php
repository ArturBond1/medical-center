...
@section('content')
    ...
    <form action="{{ route('prescriptions.update', $prescription->id) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <select name="doctor_id" id="doctor_id" class="form-control @error('doctor_id') is-invalid @enderror" required>
                ...
                @foreach($doctors as $id => $name)
                    <option value="{{ $id }}" {{ $prescription->doctor_id == $id ? 'selected' : '' }}>{{ $name }}</option>
                @endforeach
            </select>
            @error('doctor_id')
            <div class="invalid-feedback">{{ $message }}</div>
        ...
        <div class="form-group">
            ...
            <select name="patient_id" id="patient_id" class="form-control @error('patient_id') is-invalid @enderror" required>
                ...
                @foreach($patients as $id => $name)
                    <option value="{{ $id }}" {{ $prescription->patient_id == $id ? 'selected' : '' }}>{{ $name }}</option>
                @endforeach
            </select>
            @error('patient_id')
            <div class="invalid-feedback">{{ $message }}</div>
        ...
        <div class="form-group">
            ...
            <select name="medication_id" id="medication_id" class="form-control @error('medication_id') is-invalid @enderror" required>
                ...
                @foreach($medications as $id => $name)
                    <option value="{{ $id }}" {{ $prescription->medication_id == $id ? 'selected' : '' }}>{{ $name }}</option>
                @endforeach
            </select>
            @error('medication_id')
            <div class="invalid-feedback">{{ $message }}</div>
        ...
        <div class="form-group">
            ...
            <textarea name="notes" id="notes" class="form-control @error('notes') is-invalid @enderror">{{ $prescription->notes }}</textarea>
            @error('notes')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <a href="{{ route('prescriptions.index') }}" class="btn btn-secondary">Скасувати</a>
    </form>
@endsection
